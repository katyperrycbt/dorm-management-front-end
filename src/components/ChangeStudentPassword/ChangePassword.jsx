...
import { getStudentAccount, editStudentAccount } from '../../actions/student.account';
...
const Account = ({ open }) => {
    ...
    useEffect(() => {
        ...
        //get update 
        dispatch(getStudentAccount()).then((rs) => {
            if (rs.message) {
    ...
    const user = useSelector((state) => state.studentAccount);
    ...
    const handleSubmit = (e) => {
        ...
        if (formData.newPassword !== formData.reEnterNewPassword) {
            ...
            dispatch({ type: SET_LINEAR, data: true });
            dispatch(editStudentAccount(formData)).then((rs) => {
                if (rs.message) {
                    dispatch({ type: SET_LINEAR, data: false });
                    dispatch({
                        type: SET_SNACK, data: {
    ...
    return <main className={clsx(classes.content, {
        ...
        <Container maxWidth={'lg'} className={classes.container}>
            <Grid container spacing={3}>
                <Grid item xs={12}>
                    <Typography align="center" variant='h2' style={{ color: '#3f51b5' }}>Change Password</Typography>
                    <Typography align="center" variant='h5' style={{ color: '#f44336' }}>Don't give out your password to anyone, we never ask you for your password.</Typography>
                </Grid>
